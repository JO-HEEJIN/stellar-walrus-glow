...
export async function GET(request: NextRequest) {
  try {
    ...
    const simpleQuery = await db.client.query('SELECT COUNT(*) as count FROM `Product`', [])
    
    // Test 2: Get first 5 products
    const products = await db.client.query('SELECT * FROM `Product` LIMIT 5', [])
    
    // Test 3: Check table structure
    const columns = await db.client.query('SHOW COLUMNS FROM `Product`', [])
