...
export class DatabaseAdapter {
  ...
  async getAnalyticsOverview() {
    const queries = [
      ...
      { sql: 'SELECT COUNT(*) as totalOrders FROM Order_' },
      { sql: 'SELECT COALESCE(SUM(total), 0) as totalRevenue FROM Order_' }
    ]
  ...
  async createOrder(data: {
    ...
    // Insert order
    const orderSql = `
      INSERT INTO Order_ (id, userId, total, status, shippingAddress, createdAt, updatedAt)
      VALUES (?, ?, ?, ?, ?, NOW(), NOW())
    `
