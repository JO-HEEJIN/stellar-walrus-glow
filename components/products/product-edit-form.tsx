...
export default function ProductEditForm({ productId, onSuccess }: ProductEditFormProps) {
    ...
    const loadProduct = async () => {
      try {
        ...
        if (product.images && Array.isArray(product.images)) {
          setUploadedImages(product.images.map((url: string, index: number) => ({
            url,
            key: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error?.message || data.error || 'Failed to update product')
      }
